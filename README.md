...
| communal.credentialSecret | The name of a Secret that contains the credentials to connect to the communal endpoint. This may vary according to your endpoint type. For S3, the Secret must have the following keys set: <br>- *accesskey*: The access key to use for any S3 request.<br>- *secretkey*: The secret that goes along with the access key.<br><br>For example, you can create your secret with the following command:<br><pre>kubectl create secret generic s3-creds <br>--from-literal=accesskey=accesskey --from-literal=secretkey=secretkey</pre><br> For Azure, all of the credentials are stored in a Secret with the following keys: accountKey, accountName, sharedAccessSignature, and blobEndpoint. You must set accountName and one of accountKey or sharedAccessSignature. <br><br> For example, you can create your Azure secret with the following command:<br><pre>kubectl create secret generic az-creds <br>--from-literal=accountKey=key1 --from-literal=accountName=acckey </pre><br> For GCP,  you will need a HMAC key for your GCP account. Add the key pair from HMAC to a Secret with key names accesskey and secretkey. <br><br> For example, you can create GCP secret with the following command:<br><pre>kubectl create secret generic gs-creds <br>--from-literal=accesskey=key1 --from-literal=secretkey=key2 </pre><br> Then you set the Secret name in the CR, for example:<br><pre>communal:<br>  credentialSecret: s3-creds<br></pre> <br> For HDFS, .spec.communal.hadoopConfig is mandatory. <br>Then you can set the configMap name in the CR:<br><pre>communal:<br>  hadoopConfig: hadoop-conf<br></pre> <br> |  |
| communal.endpoint | The URL to the S3-compatible endpoint. The endpoint must begin with either `http://` or `https://`. This field is required and cannot change after creation. |  |
| communal.hadoopConfig | The ConfigMap with all of the hadoop config files -- it can be taken from /etc/hadoop. You can use this to setup the VerticaDB with the .spec.communal.hadoopConfig pointing to the ConfigMap that you created and .spec.communal.path referencing the namenode. For ex: `webhdfs://hdfs-ci-namenode-0.hdfs-ci-namenode.kuttl-e2e-hadoop.svc.cluster.local:50070/client-access` | |
...
| communal.path | The path to the communal storage. This may be a s3, azb or gcp or hdfs path. You can specify this respectively using the `s3://`, `azb://`, `gs://` or `webhdfs://` notation. For example: `s3://bucket-name/key-name`. You must create this bucket before creating the VerticaDB. This field is required and cannot change after creation.  If `initPolicy` is *Create*, then this path must be empty.  If the `initPolicy` is *Revive*, then this path must be non-empty. |  |
